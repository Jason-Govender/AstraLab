...
For backend work in `aspnet-core`, follow `aspnet-core/BACKEND_STRUCTURE.md` as the canonical architecture and folder-structure guide.

Key enforcement points:

- Keep the existing `AstraLab.*` project names unchanged.
...
Next.js employs a file-based routing system. In this frontend, use the App Router under `src/app` (or `app`) and follow its route, layout, loading, and error file conventions. Adhering to this convention keeps the project organized and makes route ownership easier to understand.

### Leverage Server-Side Rendering Wisely

Server rendering can improve SEO and initial page load times, but it is not necessary for every route. In the App Router, prefer async Server Components, request-aware `fetch()` usage, and route configuration over legacy Pages Router APIs.

### Embrace Static Site Generation (SSG)

Static generation offers better performance and scalability for routes with mostly stable content. In the App Router, use cached `fetch()` requests, `revalidate`, and `generateStaticParams` where appropriate instead of legacy `getStaticProps`.
...
Handle error states gracefully with App Router primitives such as `error.tsx`, `not-found.tsx`, `loading.tsx`, and clear component-level empty/error states. Providing users with informative error messages enhances user experience and helps identify and resolve issues quickly.
...
### Maintain the separation of concerns in the frontend folder structure:
...
- `hooks/` contains shared hooks used across the app when needed.
...
This file defines the provider setup style required whenever a provider module is added to this frontend.
...
This structure is mandatory whenever a provider module is introduced. Do not deviate.
...
Component and feature styling should be defined through the `style.ts` pattern using `antd-style`.
`src/app/globals.css` may remain for minimal app-wide reset and base styles.
